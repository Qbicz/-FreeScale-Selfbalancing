...
public:
    ...
    void Fetch_Values_MPU(void);

    // based on Invensense documentation, written by Filip in Dec 2015
    int mpu_read_accel_bias(int32_t* my_accel_bias_reg);
    int mpu_push_accel_bias(const int32_t* new_accel_bias);
...
// TUTAJ - NIE CHCEMY ZBIERAC DANYCH
...
// Push gyro biases to hardware registers
        ...
        blue.printf("Gyro biases:\n");
        blue.printf("dest1[0] = %f\n", dest1[0]);
        blue.printf("dest1[1] = %f\n", dest1[1]);
        blue.printf("dest1[2] = %f\n", dest1[2]);
...
// Output scaled accelerometer biases for manual subtraction in the main program
        ...
        blue.printf("Accel biases:\n");
        blue.printf("dest2[0] = %f\n", dest2[0]);
        blue.printf("dest2[1] = %f\n", dest2[1]);
        blue.printf("dest2[2] = %f\n", dest2[2]);
...
//                  -- accelerometer only --
//
void ManualCalibration(float * dest1, float * dest2) {
        uint8_t data[12]; // data array to hold accelerometer and gyro x, y, z, data
        uint16_t ii, packet_count, fifo_count;
...
// the accelerometer biases calculated above must be divided by 8.
        ...
        blue.printf("accel_bias_reg[0] = %d\n", accel_bias_reg[0]);
        blue.printf("accel_bias_reg[1] = %d\n", accel_bias_reg[1]);
        blue.printf("accel_bias_reg[2] = %d\n", accel_bias_reg[2]);
...
// Output scaled accelerometer biases for manual subtraction in the main program
        ...
        blue.printf("dest2[2] = %f\n", dest2[2]);
        */
        blue.printf("accel_bias[0] = %d\n", accel_bias[0]);
        blue.printf("accel_bias[1] = %d\n", accel_bias[1]);
        blue.printf("accel_bias[2] = %d\n", accel_bias[2]);
...
void MPU6050::Initialize_MPU(void)
{
// Read the WHO_AM_I register, this is a good test of communication
    uint8_t whoami = this->readByte(MPU6050_ADDRESS, WHO_AM_I_MPU6050);  // Read WHO_AM_I register for MPU-6050
    blue.printf("I AM 0x%x\n\r", whoami);
    blue.printf("I SHOULD BE 0x68\n\r");

    if (whoami == 0x68) { // WHO_AM_I should always be 0x68
        blue.printf("MPU6050 is online...");
        wait(1);
        blue.printf("MPU6050 OK");

        this->MPU6050SelfTest(SelfTest); // Start by performing self test and reporting values
        blue.printf("x-axis self test: acceleration trim within : %f% of factory value \n\r", SelfTest[0]);
        blue.printf("y-axis self test: acceleration trim within : %f% of factory value \n\r", SelfTest[1]);
        blue.printf("z-axis self test: acceleration trim within : %f% of factory value \n\r", SelfTest[2]);
        blue.printf("x-axis self test: gyration trim within : %f% of factory value \n\r", SelfTest[3]);
        blue.printf("y-axis self test: gyration trim within : %f% of factory value \n\r", SelfTest[4]);
        blue.printf("z-axis self test: gyration trim within : %f% of factory value \n\r", SelfTest[5]);

        wait(0.5);

        if(SelfTest[0] < 1.0f && SelfTest[1] < 1.0f && SelfTest[2] < 1.0f && SelfTest[3] < 1.0f && SelfTest[4] < 1.0f && SelfTest[5] < 1.0f) {
            // if(1) {
            this->resetMPU6050(); // Reset registers to default in preparation for device calibration
            // this->calibrateMPU6050(gyroBias, accelBias); // Calibrate gyro and accelerometers, load biases in bias registers
            this->initMPU6050();
            blue.printf("MPU6050 initialized for active data mode....\n\r"); // Initialize device for active mode read of acclerometer, gyroscope, and temperature
            wait(0.5); // wait 2
        } else blue.printf("Device did not the pass self-test!\n\r");
    } else {
        blue.printf("Could not connect to MPU6050: \n\r");
        blue.printf("%#x \n",  whoami);

        //while(1) ; // Loop forever if communication doesn't happen
        // Experiment - from https://developer.mbed.org/handbook/Debugging
        error("Communication didn't happen"); // Print a formatted message to the USB Serial Port, then die with "Siren Lights"
    }
}

void MPU6050::Fetch_Values_MPU(void)
{
// ODCZYT WARTOSCI - NIE SPRAWDZAJ CZY BIT GOTOWY
    // If data ready bit set, all data registers have new data
    //if(mpu6050.readByte(MPU6050_ADDRESS, INT_STATUS) & 0x01) {
    this->readByte(MPU6050_ADDRESS, INT_STATUS) & 0x01;
    // check if data ready interrupt
    this->readAccelData(accelCount);  // Read the x/y/z adc values
    this->getAres();
    ...
    this->readGyroData(gyroCount);  // Read the x/y/z adc values
    this->getGres();
...
/******************** Z DOKUMENTACJI INVENSENSE **************************
 * Read OTP biases from the accel bias registers
 * format G in +- 8G
 */

// my_accel_bias_reg will hold current bias values after calling this function
int MPU6050::mpu_read_accel_bias(int32_t* my_accel_bias_reg)
{
    uint8_t data[6];
    // for each exis: read 2 bytes of accel OFFSET (MSB) and put it in passed array
    readBytes(MPU6050_ADDRESS, XA_OFFSET_H, 2, &data[0]); // Read factory accelerometer trim values
    my_accel_bias_reg[0] = (int16_t) ((int16_t)data[0] << 8) | data[1];
    readBytes(MPU6050_ADDRESS, YA_OFFSET_H, 2, &data[2]);
    my_accel_bias_reg[1] = (int16_t) ((int16_t)data[2] << 8) | data[3];
    readBytes(MPU6050_ADDRESS, ZA_OFFSET_H, 2, &data[4]);
    my_accel_bias_reg[2] = (int16_t) ((int16_t)data[4] << 8) | data[5];
...
int MPU6050::mpu_push_accel_bias(const int32_t* new_accel_bias)
{
    uint32_t mask = 1uL; // Define mask for temperature compensation bit 0 of lower byte of accelerometer bias registers
    uint8_t mask_bit[3] = {0, 0, 0}; // Define array to hold mask bit for each accelerometer bias axis
    
    // create array before using function mpu_read_accel_bias
    int32_t my_accel_bias_reg[3] = {0, 0, 0};
    mpu_read_accel_bias(my_accel_bias_reg);
    
    uint8_t ii;
    for(ii = 0; ii < 3; ii++) {
        if(my_accel_bias_reg[ii] & mask) mask_bit[ii] = 0x01; // If temperature compensation bit is set, record that fact in mask_bit
    }

    // Construct total accelerometer bias, including calculated average accelerometer bias from above
    // divide by 8?
    my_accel_bias_reg[0] -= (new_accel_bias[0]); // Subtract calculated averaged accelerometer bias scaled to 2048 LSB/g (16 g full scale)
    my_accel_bias_reg[1] -= (new_accel_bias[1]);
    my_accel_bias_reg[2] -= (new_accel_bias[2]);

    uint8_t data[6]; // byte array for pushing to the registers
    data[0] = (my_accel_bias_reg[0] >> 8) & 0xFF;
    data[1] = (my_accel_bias_reg[0])      & 0xFF;
    data[1] = data[1] | mask_bit[0]; // preserve temperature compensation bit when writing back to accelerometer bias registers
    data[2] = (my_accel_bias_reg[1] >> 8) & 0xFF;
    data[3] = (my_accel_bias_reg[1])      & 0xFF;
    data[3] = data[3] | mask_bit[1]; // preserve temperature compensation bit when writing back to accelerometer bias registers
    data[4] = (my_accel_bias_reg[2] >> 8) & 0xFF;
    data[5] = (my_accel_bias_reg[2])      & 0xFF;
    data[5] = data[5] | mask_bit[2]; // preserve temperature compensation bit when writing back to accelerometer bias registers
